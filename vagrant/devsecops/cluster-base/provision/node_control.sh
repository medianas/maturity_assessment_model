#!/bin/bash
# node-control (192.168.56.5) — Ansible control-node для cluster-base v2.
# Управляет: node-gitlab / node-redmine / node-mattermost.
...
# requests нужен для ansible.uri модулей при работе с GitLab/Mattermost API
...
[gitlab_hosts]
node-gitlab ansible_host=192.168.56.10

[redmine_hosts]
node-redmine ansible_host=192.168.56.20

[mattermost_hosts]
node-mattermost ansible_host=192.168.56.30
...
echo ""
echo "[node-control] provisioning done — cluster-base scenario=$SCENARIO"
