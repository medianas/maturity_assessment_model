#!/bin/bash
# node-control (192.168.58.5) — Ansible control-node для cluster-tiers.
# Управляет node-bastion / node-internal / node-public.
...
[targets]
node-bastion  ansible_host=192.168.58.10
node-internal ansible_host=192.168.58.20
node-public   ansible_host=192.168.58.30
...
echo ""
echo "[node-control] provisioning done (cluster-tiers)"
